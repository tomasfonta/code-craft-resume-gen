					...
					<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pl-6">
						{technologies.map((tech, index) => (
							<div
								key={tech.name}
								className={`
                  ${tech.bg} ${tech.color} p-2 rounded-lg border ${isDarkMode ? 'border-gray-600' : 'border-gray-300'}
							...
							<div key={skill.name} className="flex items-center justify-between">
								...
								<div className="flex gap-1">
									{[...Array(5)].map((_, i) => (
										<div
											key={i}
											className={`w-3 h-3 rounded-full transition-all duration-300 hover:scale-125 ${i < skill.level
